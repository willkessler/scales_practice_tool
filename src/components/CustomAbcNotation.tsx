...
const CustomAbcNotation: React.FC<CustomAbcNotationProps> = ({ notation,
                                                               fretNumbers,
                                                               fingerPositions,
      ...
      try {
        ...
        setTimeout(() => {
          ...
          if (svg) {
            ...
            highlightNote(svg, highlightedNoteIndex);
          } else {
  ...
  }, [notation, highlightedNoteIndex]);

  const getNoteNamesFromAbc = (abcNotation: string): string[] => {
  ...
  const highlightNote = (svg: SVGSVGElement, index: number) => {
    ...
    if (noteElements.length > index) {
      ...
      highlight.setAttribute('fill', '#0f0');
      highlight.setAttribute('opacity', '0.5');
      highlight.setAttribute('stroke', '#0f0');
      group.appendChild(highlight);

      // Get the note names
      const noteNames = getNoteNamesFromAbc(notation);
      console.log('noteNames:', noteNames);
      const noteName = noteNames[index] || `Note ${index + 1}`;
      //const fretNumbers = getFretNumbersFromAbc(notation);
      ...
      label.setAttribute('y', (bbox.y + bbox.height + 15).toString());
      ...
      if (fingerPosition !== null) {
        ...
        fingerCircle.setAttribute('cy', (bbox.y + bbox.height + 27).toString());
        ...
        label.setAttribute('class', 'note-label');
        label.setAttribute('x', (bbox.x + bbox.width / 2).toString());
        label.setAttribute('y', (bbox.y + bbox.height + 30).toString());
        label.setAttribute('text-anchor', 'middle');
        label.setAttribute('font-size', '10');
